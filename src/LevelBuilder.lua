...
function LevelBuilder:update(dt)
    if love.mouse.wheelmoved ~= 0 then
        self:scroll(love.mouse.wheelmoved)
    end

    if love.mouse.wasPressed(1) then
        self.sx, self.sy = tilemath.snap(self.level.tileSize, love.mouse.getX(), love.mouse.getY())    
    end
    ...
    if love.mouse.isDown(1) then
        self.ex, self.ey = tilemath.snap(self.level.tileSize, love.mouse.getX(), love.mouse.getY())
